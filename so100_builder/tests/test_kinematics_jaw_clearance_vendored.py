"""Validates jaw_clearance.py -- the swept-jaw collision pre-filter -- runs
correctly against the VENDORED copy at so100_builder/kinematics/, not just
the so_arm_100_kinematics ROS package.

This is the addon's own counterpart to so_arm_100_kinematics/test/test_jaw_clearance.py
(same cases, same numbers), re-pointed to so100_builder.kinematics per the
so_arm_100_kinematics README's "Vendoring into the Blender addon" rule 4:
after vendoring, re-run the tests against the vendored copy -- if they
don't pass in Blender's interpreter, the copy is wrong. See
test_kinematics_vendored.py/test_kinematics_grasp_vendored.py, which do
the same for chain.py/grasp.py; this is the missing third one, added when
jaw_clearance.py itself was vendored for the first time (it shipped in
so_arm_100_kinematics 1.2.0, but that version was never vendored here
until now).
...
Deliberately plain unittest, matching the other vendored test files' own
...
from so100_builder.kinematics.so_arm_100.constants import (
    GRASP_OFFSET_M,
    JAW_RADIUS_M,
    STICK_COLLISION_RADIUS_M,
)
from so100_builder.kinematics.so_arm_100.jaw_clearance import (
    check_jaw_clearance,
    jaw_swept_capsule,
    segment_distance,
)
