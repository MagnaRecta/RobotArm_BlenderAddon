...
The addon is not tied to one robot (BRIDGE_PROTOCOL.md Sec A.1.1) --
`so100_builder/kinematics/` holds one vendored sub-directory per robot id,
and this package's id is `so_arm_100`. Copy the inner module directory, not
the ROS package wrapper:

```
so_arm_100_kinematics/so_arm_100_kinematics/*.py   ->   so100_builder/kinematics/so_arm_100/
so_arm_100_kinematics/VERSION                      ->   so100_builder/kinematics/so_arm_100/VERSION
```
