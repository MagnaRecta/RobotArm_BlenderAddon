...
from .constants import CHAIN, GRASP_OFFSET_M, JAW_CONTACT_HALF_LENGTH_M, MIN_GRASP_OFFSET_M
...
def grasp_offset_for_length(length_m, default_offset_m=GRASP_OFFSET_M,
    ...
    this length -- D13 (ROS2_IMPLEMENTATION_PLAN.md §4): below some length,
    the fixed ``GRASP_OFFSET_M`` would close the jaws past the stick's own
    tip, so a shorter stick needs a smaller offset.

    Always returns the LARGEST offset that still keeps the jaws fully on the
    stick -- not merely a value below the tip. The offset is measured up
    from the base, so a larger offset means the jaws sit higher above the
    feeder floor (more clearance), never less -- this also means every
    stick long enough for the unmodified ``default_offset_m`` (today,
    anything >= ~56mm) gets exactly that value back, unchanged.

    Raises :class:`chain.Unreachable` if even the largest possible offset
    falls below ``min_offset_m`` -- the stick is too short to grip safely
    with any offset, not merely too short for the default one.
    """
