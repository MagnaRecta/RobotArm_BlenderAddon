...
class TestGraspOffsetForLength(unittest.TestCase):
    """D13: the grasp offset adapts to short sticks -- ROS2_IMPLEMENTATION_PLAN.md §4."""

    def test_normal_length_stick_gets_the_unmodified_default(self):
        # Every currently-tested stick (80-150mm) must get back exactly
        # GRASP_OFFSET_M, unchanged -- this is the zero-regression guarantee
        # the whole design relies on.
        self.assertEqual(grasp_offset_for_length(0.110), GRASP_OFFSET_M)
        self.assertEqual(grasp_offset_for_length(0.150), GRASP_OFFSET_M)

    def test_fifty_mm_stick_gets_the_physical_ceiling(self):
        # length - jaw_contact_half_length = 0.050 - 0.005 = 0.045 -- the
        # user's own "physical maximum" example.
        self.assertAlmostEqual(grasp_offset_for_length(0.050), 0.045, places=9)

    def test_offset_never_exceeds_length_minus_jaw_contact_half_length(self):
        for length_m in (0.060, 0.070, 0.080, 0.100):
            offset = grasp_offset_for_length(length_m)
            self.assertLessEqual(offset, length_m - JAW_CONTACT_HALF_LENGTH_M)

    def test_too_short_to_grip_safely_raises_unreachable(self):
        # An offset below MIN_GRASP_OFFSET_M means even the largest usable
        # offset for this length isn't trusted -- genuinely too short.
        too_short_m = MIN_GRASP_OFFSET_M + JAW_CONTACT_HALF_LENGTH_M - 0.001
        with self.assertRaises(Unreachable):
            grasp_offset_for_length(too_short_m)
